...
const ApiRoot = "https://selly.io/api/v2/"
...
func (s *Selly) GetProduct(id string) (*Product, error) {
	furl := "%sproducts/%s"
	b, err := s.httpClient.GetBytes(fmt.Sprintf(furl, ApiRoot, id))
	if err != nil {
		return nil, err
	}
	p := Product{}
...
func (s *Selly) GetProducts() ([]Product, error) {
	furl := "%sproducts"
	b, err := s.httpClient.GetBytes(fmt.Sprintf(furl, ApiRoot))
	if err != nil {
		return nil, err
	}
	p := []Product{}
...
// NewProduct creates a new product and returns a product or error
func (s *Selly) NewProduct(params ProductParams) (*Product, error) {
	url := "%sproducts"
	product := Product{}
	...
	resp, err := s.postreq(fmt.Sprintf(url, ApiRoot), data)
...
func (s *Selly) DeletePayment(id string) error {
	furl := fmt.Sprintf("https://selly.io/api/v2/pay/%s", id)
